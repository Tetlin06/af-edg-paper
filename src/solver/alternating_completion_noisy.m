...
% --------------------------------------------------------------------------
function [Global_Coordinate, IPM_Recon, output]=alternating_completion_noisy(Dist,Weight,Initial,opts,lsopts)
...
% -------------------------------------------------------------------------
    function [F,G] = gradient(P)
        tmp0 = A_operator(P)-b+D1;
        tmp = w .* tmp0;
        % F is the objective function
        F = sum(sum(P.*P,2))+0.5*r*norm(tmp,'fro')^2;
        % G is the gradient
        G = 2*P+2.0*r*At_operator((w.^2).*tmp0)*P;
    end
    ...
    function [F,G] = gradient2(P)
        ...
        tmp = w .* tmp0;
        % F is the objective function
        F = 0.5*r*norm(tmp,'fro')^2;
        % G is the gradient
        G = 2.0*r*At_operator((w.^2).*tmp0)*P;
    end
end
